...
function buscar(){
	...
	$.ajax({
		...
		var datos = [];
		$.each(response.data, function(k,v){
			var tbl = [v.SUB, v.RAZON_SOCIAL, v.factura, v.iccid, v.material, v.imei, v.imsi, v.fecha_doc ];
			datos.push(tbl);
		})
		if(tabla){
			tabla.destroy();
		...
		var columnas = ["Sub Distribuidor","Vendedor","Factura", "Iccid","Material","Imei","Imsi","Fecha Documento"];
		var finalColumn = [];
