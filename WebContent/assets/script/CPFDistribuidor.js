...
function buscar(){
	...
	$.ajax({
		...
		datos = [];
		$.each(response.data, function(k,v){
			var tbl = [v.PROVEEDOR, v.DISTRIBUIDOR,v.FACTURA, v.fecha_doc, v.fecha, v.Q_RECEPCION, v.HABILITADOS,v.RECARGA_30_60, v.PENDIENTES_HABILITAR, v.Q ];
			datos.push(tbl);
		})
		if(tabla){
			tabla.destroy();
		...
		columnas = ["Proveedor", "Distribuidor","Factura","Fecha Documento", "Fecha","Cantidad Recepcion","Habilitados", "Recargas al 30-60" ,"Pendientes X Habilitar","QPDV"];
		$('#tbl_RendimientoVlidadr').html('<tfoot><tr><th colspan="5" style="text-align:right">Total:</th><th></th><th></th><th></th><th></th></tr></tfoot>')
		var finalColumn = [];
		...
		tabla = $('#tbl_RendimientoVlidadr').DataTable({
			...
			footerCallback: function ( row, data, start, end, display ) {
	            ...
	            tQRecepcon = api.column(5).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            yHabilitados = api.column(6).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            tpHabilitados = api.column(7).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
//	            tCantidad = api.column(8).data().reduce( function (a, b) {
//                    return intVal(a) + intVal(b);
//                }, 0 );
	            $( api.column(5).footer() ).html(tQRecepcon);
	            $( api.column(6).footer() ).html(yHabilitados);
	            $( api.column(7).footer() ).html(tpHabilitados);
