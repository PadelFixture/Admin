...
//	}
	var input = {
        SP: "CONSULTA_HABILITADOS_COMERCIO",
        FILTERS: [
            {value: comercio},
            {value: 3},
	...
	$.ajax({
		...
		datos = [];
		$.each(response.data, function(k,v){
			var tbl = [v.PROVEEDOR, v.DISTRIBUIDOR, v.SUBDISTRIBUIDOR, v.VENDEDOR, v.COMERCIO,v.FECHA,v.REQUESTDATE, v.factura, v.iccid, v.material, v.imei, v.imsi, v.fecha_doc ];
			datos.push(tbl);
		})
		if(tabla){
			tabla.destroy();
		...
		columnas = ["Proveedor", "Distribuidor", "Sub Distribuidor","Vendedor","Comercio","FECHA MOVIMIENTO","FECHA HABILITADO","Factura", "Iccid","Material","Imei","Imsi","Fecha Documento"];
		var finalColumn = [];
