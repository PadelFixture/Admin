...
function buscar(){
	...
	$.ajax({
		...
		var datos = [];
		$.each(response.data, function(k,v){
			var tbl = [v.DISTRIBUIDOR, v.SUBDISTRIBUIDOR, v.CANTIDAD];
			datos.push(tbl);
		})
		if(tabla){
			tabla.destroy();
		...
		$("#tbl_RendimientoVlidadr").html('<tfoot><tr><th colspan="2" style="text-align:right">Total:</th><th></th></tfoot>')
		var columnas = ["Distribuidor", "Sub Distribuidor","Cantidad"];
		var finalColumn = [];
		...
		tabla = $('#tbl_RendimientoVlidadr').DataTable({
			...
			footerCallback: function ( row, data, start, end, display ) {
	            ...
	            var cantidadTotal = api.column(2).data().reduce( function (a, b) {
