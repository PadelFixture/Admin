...
function buscar(){
	...
	getData(input).then(function(response){
		...
		$.each(response.data, function(k,v){
			...
			t1 += total;
			t2 += toActi;
			var tbl = [v["COMERCIO"],v["GIRO"],v["DIRECCION"],v["COMUNA"], v["TELEFONO"],  total, toActi];
			datos.push(tbl);
		})
		if(tabla){
			tabla.destroy();
		...
		$("#tbl_RendimientoVlidadr").html('<tfoot><tr><th colspan="5" style="text-align:right">Total:</th><th></th><th></th></tr></tfoot>');
		columnas = ["Comercio/Vendedores","Giro","Direccion","Comuna","Contacto","Q SS Total", "Total actividad"];
		var finalColumn = [];
		...
		tabla = $('#tbl_RendimientoVlidadr').DataTable({
			...
			footerCallback: function ( row, data, start, end, display ) {
	            ...
	            t1 = api.column(5).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            t2 = api.column(6).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            $( api.column(5).footer() ).html(t1);
	            $( api.column(6).footer() ).html(t2);
	        }
		});
