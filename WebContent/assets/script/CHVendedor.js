...
function buscar(){
	...
	var input = {
        SP: "CONSULTA_HABILITADOS_VENDEDOR",
        FILTERS: [
            {value: vendedor},
            {value: 2},
	...
	$.ajax({
		...
		datos = [];
		$.each(response.data, function(k,v){
			var tbl = [v.PROVEEDOR,v.DISTRIBUIDOR, v.SUBDISTRIBUIDOR, v.VENDEDOR,v.FECHA,v.REQUESTDATE, v.factura, v.iccid, v.material, v.imei, v.imsi, v.fecha_doc ];
			datos.push(tbl);
		})
		if(tabla){
			tabla.destroy();
		...
		columnas = ["Proveedor", "Distribuidor", "Sub Distribuidor","Vendedor","FECHA MOVIMIENTO","FECHA HABILITADO","Factura", "Iccid","Material","Imei","Imsi","Fecha Documento"];
		var finalColumn = [];
