...
//			db.close();
	...
	public static String getParameterNameSp (String sp)throws Exception{
		...
		JSONArray columns = new JSONArray();
		try {
			String getSp = "SELECT *FROM information_schema.parameters WHERE SPECIFIC_NAME = '"+sp+"';";
	...
	public static String Select (String row)throws Exception{
		...
		try {
			...
			if(json.has("WHERE")){
				...
				for(int x = 0; x < tColumnName.size(); x++){
					if(e.has(tColumnName.get(x))){
						where += cWhere == 0? tColumnName.get(x) +" = '"+e.get(tColumnName.get(x))+"'": " AND "+tColumnName.get(x) +" = '"+e.get(tColumnName.get(x))+"'";
						cWhere++;
					}
				}
			}
...
//			ps.close();
	...
	public static String CallSpDb (ConnectionDB db, Statement multiQuery)throws Exception{
		JSONObject json = new JSONObject();
//		multiQuery.executeBatch();
		try {
			multiQuery.executeBatch();
			...
			json.put("message_1", e);
			json.put("message_2", e.getStackTrace());
			json.put("error", 2);
			cdb.conn.rollback();
			cdb.close();
			cdb = null;
