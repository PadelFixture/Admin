...
function buscar(){
	...
	getData(input).then(function(response){
		...
		$.each(response.data, function(k,v){
			...
			totalGeneral += toActi;
			totalQss += total;
			totalTotal += toActi + total;
			totalOtro += v["OTRO"];
			totalQpp += v["Q PP"];
			totalQportal += v["Q PORTAS"];
			totalQss1 += v["Q SS 14.990"];
			totalQss2 += v["Q SS 9.990"];
			var tbl = [v["DISTRIBUIDOR"],v["SUBDISTRIBUIDOR"],v["VENDEDOR"],v["COMERCIO"],v["USERNAME"], v["Q SS 9.990"], v["Q SS 14.990"], v["Q PORTAS"], v["Q PP"], v["OTRO"],v["PLAN FACIL"], total, toActi];
			datos.push(tbl);
		})
		if(tabla){
			tabla.destroy();
		...
		$('#tbl_RendimientoVlidadr').html('<tfoot><tr><th colspan="5" style="text-align:right">Total:</th><th></th><th></th><th></th><th></th><th></th><th></th><th></th><th></th></tr></tfoot>')
		columnas = ["Distribuidor","Subdistribuidor","Vendedor","Comercio/Vendedores","USERNAME","Q SS 9.990","Q SS 14.990", "Q PORTAS", "Q PP", "Otro", "Plan fácil","Q SS Total", "Total actividad"];
		var finalColumn = [];
		...
		tabla = $('#tbl_RendimientoVlidadr').DataTable({
			...
			footerCallback: function ( row, data, start, end, display ) {
	            ...
	            totalQss2 = api.column(4).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            totalQss1 = api.column(5).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            totalQportal = api.column(6).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            totalQpp = api.column(7).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            totalOtro = api.column(8).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            totalQss = api.column(9).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            totalPFacil = api.column(10).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            totalGeneral = api.column(11).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            totalTotal = api.column(12).data().reduce( function (a, b) {
                    return intVal(a) + intVal(b);
                }, 0 );
	            //$( api.column(4).footer() ).html(totalQss2);
	            $( api.column(5).footer() ).html(totalQss1);
	            $( api.column(6).footer() ).html(totalQportal);
	            $( api.column(7).footer() ).html(totalQpp);
	            $( api.column(8).footer() ).html(totalOtro);
	            $( api.column(9).footer() ).html(totalQss);
	            $( api.column(10).footer() ).html(totalPFacil);
	            $( api.column(11).footer() ).html(totalGeneral);
	            $( api.column(12).footer() ).html(totalTotal);
	        }
		});
